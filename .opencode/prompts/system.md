...
- 先读取：`AGENTS.md`、`readme.md`、`opencode.jsonc`、`.opencode/README.md`、`.codex/README.md` 与 `.codex/instructions.md`（Codex 场景）、`.opencode/prompts/repo_context.md`。
- 修改 Gradle 必须读取：`.opencode/skills/gradle-spec/SKILL.md`、`settings.gradle`、根 `build.gradle`、`gradle.properties`、`buildSrc/build.gradle`、`buildSrc/gradle.properties`、`buildSrc/common-*`。
- 发布链路必须保留：`io.geewit.publish.enabled`、`maven-publish`、`signing`、`org.jreleaser`、`staging-deploy`、Central Bearer token 凭据路径。
